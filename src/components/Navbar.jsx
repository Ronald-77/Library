...
export default function Navbar() {
    ...
    const { isLoggedIn, isAdmin, setUser, clearUser } = useSession();

    useEffect(() => {
        setUser(); // Initialize session state

        const handleStorageChange = () => {
            setUser(); // Update session state on storage changes
        };
            ...
            <ul className='flex justify-between items-center p-3 max-w-6xl mx-auto'>
                ...
                <li className='flex gap-3 items-center'>
                    ...
                    <div className='cursor-pointer'>
                        {isDark ? <img src={lightIcon} alt="" onClick={() => { changeTheme('light') }} /> : <img src={darkIcon} alt="" onClick={() => { changeTheme('dark') }} />}
                    </div>
                </li>
            </ul>
        </nav>
